...
HTML pages are in the [`/source`][source-dir] directory and are authored using
Markdown or ERB templates. Make edits in a branch and open a pull request.

### HTML pages

To update page content (index, getting started, reference, etc.), edit the
corresponding `.md` or `.md.erb` file under `source/`:
...
`source/openapi.yaml` is the v2 API specification. It is published at
`/openapi.yaml` and used to build `llms-full.txt` at site build time.

**Do not edit this file directly.** It is synced automatically from
[`trade-tariff-backend/swagger/v2/swagger.json`][backend-swagger] by the [Update
OpenAPI spec][update-workflow] workflow, which runs every Monday and opens
a pull request if the spec has changed. You can also trigger it manually via
`workflow_dispatch`.
...
and no HTML artefacts. Anchors become Markdown links and `<code>` spans become
backticks. They are intended for direct consumption by LLM clients and coding
assistants, and are listed in [`llms.txt`](source/llms.txt).

Each page also has a "View as Markdown" link in its aside.

The `.md` files are generated at build time by `config.rb`. To add a new page,
create the source file under `source/` with a `.html.md` or `.html.md.erb`
extension — the Markdown URL is produced automatically.
...
The server runs at [http://localhost:4567](http://localhost:4567). HTML pages
reload automatically on save. Restart the server after changes to ERB templates
that load data files (e.g. `llms-full.txt.erb`).

### VS Code Dev Container

Open the folder in VS Code, then run **Dev-Containers: Open Folder in
Container** from the command palette. The container builds and starts the server
automatically at <http://localhost:4567>.

## Layout overrides (`govuk_tech_docs`)

This site overrides the Tech Docs gem's `_header.erb` and `_footer.erb`
under `source/layouts/`. Header links and footer URLs are configured in
`config/tech-docs.yml`.

After `bundle update govuk_tech_docs`, run `bin/tech-docs-layout-upstream-hint`;
